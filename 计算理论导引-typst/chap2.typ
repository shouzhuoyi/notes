...
#definition[
  可计算全函数$f:{0,1}^* arrow.r {0,1}^*$是问题A到问题B的*m-归约*,记为$A lt.eq.slant_m B$,若满足:$forall x in {0,1}^*,f(x)in B <==> x in A$.
]

我们称$B$是对数空间规约到$C$的,当且仅当B到C的规约是隐式空间可计算的.对数空间规约记作$lt.eq.slant_L$.
...
#proof[
  ...
  2. 证明 $"2SAT"$ 是 #nl()-难的.这个证明极其冗长(惩罚你去书上找推论1.4)
    #lemma[
      有向无环图的可达性问题是#nl()-完全的.
      #line()
      #proof[
        如果我们把一个对数空间的非确定图灵机和一个多项式时间的时钟图灵机硬连接,我们得到了一个每条计算路径都终止的图灵机.它的格局图是有向无环图.计算这个可达性问题自然就和计算图灵机的计算路径一致.所以它是#nl()-难的,进而完全的.
      ]
    ]
    记$G$是一个有向无环图.我们构造规约$f : G |->"2SAT"$来说明命题.对$s,t in V,$$ phi_G eq.delta {s or overline(t) or or.big (overline(x) or y) | ⟨x,y⟩ in E} $.
    那么$phi_G in "2SAT"$,并且容易验证这就是个规约,命题得证

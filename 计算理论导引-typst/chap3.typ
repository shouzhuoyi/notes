...
#proof[
  说白了是添加垃圾.若$L ∈ bold("NEXP")$ 在 $2^n^c$ 时间内被计算,那么,用垃圾信息填充:

  $ L^star = { x 01^(2^(|x|^c)) | x in L } in #np() $
  把每个输入 x 后面加上一串很长的 01 串,长度是 $2^(|x|^c)$（指数级长）.现在$L^star$已经属于#np()了.
  那么根据假设,$L^star in #p() ==> L in bold("EXP")$.
]

#remark[
  读者应当把这个添加垃圾信息与后面的Ladner定理扯上关系.🍬
]
...
那么,记作$A ≅_p B$.我不想看Myhill自同构定理,但是他们证明了$exists "Karp reductions" k,k^(-1),==> A ≅_p B$.
...
#proof[
  ...
  2. $H$可在多项式时间内计算.
    ...
    - 我们最多需要检查 $log log n$ 个图灵机,每个图灵机要检查所有满足$|x| <= log(n)$的|x|,这样的$x$有$2^log(n) = O(n)$个(外面的$O(n)$).
      ...
      同时,我们需要递归的看$x = phi.alt 01^(abs(phi.alt)^(H(abs(phi.alt))))$里的$phi.alt in_? H$.而$abs(phi.alt) = o(log(abs(x)))$的.所以,$     & T(n) lt.slant (log log (n))O(n)(o(n)+O(n)+T(log(n))+O(1)) \
      ==> & T(n) = o(n^3). $(直接从上式好像不好看出为啥是$o(n^3)$的.但我们可以待定一下系数,例如$T(n) = o(n^k) lt.slant o(n^k)$,通过归纳法确定$k$.事实上$T(n)$比较接近$n^2 "poly"(n)$,这里单纯只是为了证明比较方便)
    ...
    若$"SAT"_H in bold("NPC")$,则存在一个在$d n^d$时间内计算的规约$r: "SAT"|-> "SAT"_H$.那么,$H(n)$是没有上界的,否则和上述证明一样了,矛盾.

    那么,由上述,$exists N , abs(psi)>d and$
    $ abs(r(phi)) = abs(psi 01^(abs(psi)^(H(abs(psi))))) >N and H(abs(psi)) >2d+1 $.
...
#proof[
  ...
  #line()
  2. 第二个case:直观地想,如果有一个问题,拒绝它需要调用大于多项式次数的神谕,所以我们在调用多项式次数的情形下*会犯错*,那么构造这样的问题就好了.不妨设这样的神谕是$B$,并且有$B_0 subset.eq B_1 subset.eq B_2 dots.c ,B = ⋃ B_i$.并同时$n_0 lt n_1 lt n_2 dots.$这里$n_(i+1)$只和调用神谕的时候*询问*$a in_? B_i$的$a$有关.有可能$a$非常长,但我们只需要$n_(i+1)$比所有可能的$a$更长.
  ...
  3. 证明$L_B in #np()^B,L_B in.not #p()^B$.前者已经在证明的细节里提到了.后者:反证法.如果$MM_i^B$在$T(n)$时间内判定$L_B$,取i充分大,一定有$T(n_i) lt 2^(n_i-1)$.按定义,$ MM_i^B (1^(n_(i+1))) = 0 & <==> forall x in B,abs(x) eq.not n_(i+1) \
                             & <==> forall x in B_(i+1),abs(x) eq.not n_(i+1) \
                             & <==> MM_(i)^(B_(i+1)) (1^(n_(i+1))) = 1 \
                             & <==> MM_i^B (1^(n_(i+1))) = 1 $
  矛盾.
]

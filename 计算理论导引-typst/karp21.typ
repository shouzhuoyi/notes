...
#proof[
  1. SAT $lt.slant_K$ 0-1 Integer Programming. 
      - 对于 SAT 实例中的每一个布尔变量 $X_i$，在整数规划中创建一个对应的二进制变量 $x_i in {0, 1}$。如果 $X_i$ 为真，对应 $x_i = 1$。如果 $X_i$ 为假，对应 $x_i = 0$。$X_i$转化为$x_i$。$not X_i$ 转化为$(1 - x_i)$。
      - 子句转化SAT要求公式中的每一个子句都必须为真；一个子句只要其中有一个文字为真即可满足。
      ...
      - 如果原 SAT 公式 $S$ 有 $m$ 个子句，我们就会得到 $m$ 个线性不等式。SAT是可满足的 $arrow.l.r.double.long$ 这组 0-1 不等式组有可行解。

...
#problem[
  - SAT
    - 0-1 Integer Programming (0-1 整数规划：在变量仅取0或1的约束下，寻找线性目标函数的最优解)
    - Clique (最大团问题：在图中寻找一个最大的节点子集，使得其中任意两点间都有边相连)=
      - Set Packing (集合包装问题：给定若干集合，找出一组互不相交的子集，使其数量最大)
      - Vertex Cover (顶点覆盖问题：寻找最小的节点集合，使得图中每条边都至少有一个端点在该集合中)
        - Set Covering (集合覆盖问题：用最少数量的子集来覆盖全集中的所有元素)
...
#proof[
  ...
  5. Vertex Cover $lt.slant_K$ Set Covering.$forall G = angle.l V,E angle.r in "Vertex Cover"$,$cal(U) = overline(E),S_v eq.delta {e in cal(U) | v in e}$#line()

...
#remark[
  ...
  $L$可以视为一类问题的有解实例的集合，例如“所有取值可能为1的主合取范式”等，这些主合取范式构成了集合$L$。

  参考书例子: 回文串 (朴素, P8).

  一个问题是#strong[图灵机可解的] $<==>$ 有一台图灵机计算它.
]
...
#remark[
  时间可构造性是指,$T(n)$本身可以被一个图灵机在接收$1^n$(表示n这个数)的输入后,在有限步($O(T(n))$)内,输出$n$的二进制表示.\
...
#remark[
  存在一台通用图灵机,它可以模拟任何一台图灵机.这是说：对于任意的输入,上述两台图灵机的行为完全等价（产生的输出完全相同或者同时永不停机）；代价是一点额外的时间.
]
...
#problem(" decidable")[
  UC问题:
  $ cal(U C) (alpha) = cases(delim: "{", 0","& "if " MM_alpha (alpha) = 1, 1 ","& upright("otherwise"),) $
  停机问题：
  $
...
#remark[
  否定性命题的一般证明方法就是利用反证法,其中对角线方法给了很好的例子.
]

== 加速定理 <加速定理>
加速定理的动机在于思考:对于一个问题,有没有最好的算法解决它?
我们现在知道,结论否定(*Blum’s Speedup Theorem*).#text("为了证明他,我们不证明他", blue) .转而看向线性加速定理.
...
#proof[
  ...
  中间的包含关系是说,非确定性图灵机（NTM）在多项式时间内能解决的问题,可以用确定性图灵机在指数时间内用暴力算法找出所有的格局($2^n$)模拟出来.根据非确定性图灵机的定义,这也是可以理解的.
]

...
<dijkstra-算法>
#block[
  $d i s t \[ s \] arrow.l 0$ \$Q \\gets \\textsc{MinPriorityQueue}(V)\$
  $\( d i s t \, p r e v \)$
...
=== 活动选择问题
<活动选择问题>
你有n个活动构成的集合$S = { a_1 \, a_2 \, dots.h \, a_n }$，这些活动都需要占用会议室，且每个时刻会议室只能给一场活动提供服务。每个活动$a_i$的开始时间(start
time)为$s_i$，结束时间(finish
time)为$f_i$，其中$0 lt.eq s_i < f_i < oo$。如果活动$a_i$占用的时间为左闭右开区间$\[ s_i \, f_i \)$。如果活动$a_i$和$a_j$的时间区间$\[ s_i \, f_i \)$和$\[ s_j \, f_j \)$没有重叠，那么活动$a_i$和$a_j$相互兼容。活动选择问题目标是从所有活动中选出一个总时间规模最大相互兼容的活动子集。假设这些活动已经按照结束时间从小到大排序，即：$f_1 lt.eq f_2 lt.eq f_3 lt.eq dots.h.c lt.eq f_n$

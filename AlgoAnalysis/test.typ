...
则称$f \( n \)$是$O \( g \( n \) \)$的，记为$f \( n \) = O \( g \( n \) \)$#footnote[这种\"=\"的记号的本意其实是$f \( n \) in O \( g \( n \) \)$，而对于出现在等式或不等式中的渐进记号表示的也是，其中的某一函数]。也可以这样理解，当$lim_(n arrow.r oo) frac(f \( n \), g \( n \))$存在且不为无穷，那么就有$f \( n \) = O \( g \( n \) \)$，(但是ppt上似乎还要求极限不为无穷，私以为这里把极限为无穷也算作了极限存在的一种)

=== $Omega$记号：渐进下界记号
<omega记号渐进下界记号>
设$f \( n \)$和$g \( n \)$是从自然数到非负正数集上的两个函数，如果存在正常数$c$和自然数$n_0$，使得
$ forall \, gt.eq n_0 \, f \( n \) gt.eq dot.op g \( n \) $
则称$f \( n \)$是$Omega \( g \( n \) \)$的，记为$f \( n \) = Omega \( g \( n \) \)$。也可以这样理解，当$lim_(n arrow.r oo) frac(f \( n \), g \( n \))$存在且不为0，则表示$f \( n \) = Omega \( g \( n \) \)$

=== $Theta$-记号：紧渐进界记号
<theta-记号紧渐进界记号>
如果存在2个正常数$c_1 \, c_2$和自然数$n_0$，使得
$ forall n gt.eq n_0 \, c_1 dot.op g \( n \) lt.eq f \( n \) lt.eq c_2 dot.op g \( n \) $
则称$f \( n \)$是$Theta \( g \( n \) \)$，记为$f \( n \) = Theta \( g \( n \) \)$，也就是说，$lim_(n arrow.r oo) frac(f \( n \), g \( n \)) = c > 0$, 则$f \( n \) = Theta \( g \( n \) \)$

可以推断，$f \( n \) = Theta \( g \( n \) \) arrow.l.r.double f \( n \) = O \( g \( n \) \) and f \( n \) = Omega \( g \( n \) \)$, 此时称$f \( n \)$与$g \( n \)$同阶，

=== $o$-记号
<o-记号>
对于任意给定的$c > 0$，都存在正整数$n_0$，使得当$n gt.eq n_0$时有
$ f \( n \) < c dot.op g \( n \) $
则称函数$f \( n \)$是$o \( g \( n \) \)$的，记为$f \( n \) = o \( g \( n \) \)$也就是说$lim_(n arrow.r oo) frac(f \( n \), g \( n \)) = 0$，则$f \( n \) = o \( g \( n \) \)$.

因此可以有：
$ f \( n \) = o \( g \( n \) \) arrow.l.r.double f \( n \) = O \( g \( n \) \) and g \( n \) eq.not O \( f \( n \) \) $
当$n$充分大时，$f \( n \)$的阶比$g \( n \)$低。

=== $omega$-记号：非紧下界记号
<omega-记号非紧下界记号>
对于任意给定的$c > 0$，都存在正整数$n_0$，使得当$n gt.eq n_0$时有$f \( n \) > dot.op g \( n \)$，则称函数$f \( n \)$时$omega \( g \( n \) \)$的，记为$f \( n \) = omega \( g \( n \) \)$,当$lim_(n arrow.r oo) frac(f \( n \), g \( n \))$不存在，$lim_(n arrow.r oo) frac(f \( n \), g \( n \)) arrow.r oo \, f \( n \) = omega \( g \( n \) \)$.
...
#figure(
  align(center)[#table(
    ...
    table.header([#strong[OP]], [#strong[Real Cost] $bold(C_(O P))$], [#strong[Amortized Cost] $bold(hat(C)_(O P))$]),
    table.hline(),
...
<主定理>
#block[
  设$a gt.eq 1 \, b gt.eq 1$为常数，$f \( n \)$为函数, $T \( n \)$为非负正数，且
  $ T \( n \) = a T \( n / b \) + f \( n \) $
  那么就有
...
对于$T \( n \) = 2 T \( n / 2 \) + n log n$, 就是不能使用主定理的，因为不存在$epsilon > 0$使得$n log n = Omega \( n^(1 + epsilon) \)$.
...
=== 应用示例
<应用示例>
分析递归式 $T \( n \) = T \( floor.l n \/ 2 floor.r \) + T \( ceil.l n \/ 2 ceil.r \) + n$：
...
=== 势函数规范化构造及其平摊代价不变性证明
<势函数规范化构造及其平摊代价不变性证明>
假定有势函数 $Phi$，对所有 $i gt.eq 1$ 满足 $Phi \( D_i \) gt.eq Phi \( D_0 \)$，但 $Phi \( D_0 \) eq.not 0$。证明：存在势函数 $Phi'$，使得 $Phi' \( D_0 \) = 0$，对于所有 $i gt.eq 1$，满足 $Phi' \( D_i \) gt.eq 0$，且使用 $Phi'$ 的平摊代价与使用 $Phi$ 的平摊代价相同。
...
因此平摊代价为：
$ frac(T \( n \), n) lt.eq 3 = O \( 1 \) $

=== 主定理运用
<主定理运用>
设递推方程 $T \( n \) = 7 T \( n / 2 \) + n^2$ 给出了算法$A$在最坏情况下的时间复杂度函数，算法$B$在最坏情况下的时间复杂度函数 $W \( n \)$ 满足递推方程 $W \( n \) = a W \( n / 4 \) + n^2$。试确定最大的正整数 $a$，使得 $W \( n \)$ 的阶低于 $T \( n \)$ 的阶。

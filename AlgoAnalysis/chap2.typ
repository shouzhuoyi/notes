...
解得
$ h \( n \) = h \( 1 \) + sum_(i = 1)^(n - 1) frac(1, i !) < sum_(i = 1)^oo = e - 1 $
所以
$ f \( n \) = n ! h \( n \) < \( e - 1 \) n ! < 2 dot.op n ! $
...
=== 基数排序
<基数排序>
令 $L = { a_1 \, a_2 \, dots.h \, a_n }$ 是一张有 $n$ 个数的集合，其中每个数恰好有 $k$ 位数字，就是说每个数具有 $d_k d_(k - 1) dots.h d_1$ 形式，这里 $d_i$ 是 $0$ 到 $9$ 中的一个数字。
...
=== 寻找多数元素
<寻找多数元素>
令 $A \[ 1 . . n \]$ 是一个整数序列，$A$ 中的整数 $a$ 如果在 $A$ 中出现的次数多于 $floor.l n \/ 2 floor.r$，那么 $a$ 称为多数元素。
